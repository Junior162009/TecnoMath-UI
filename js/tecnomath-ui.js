(() => {
  ...
  // Los juegos siguen viviendo en MindMathArcade. Se abren desde su origen
  // para conservar sus HTML, CSS, JS, imágenes, fuentes, manifest y demás assets.
  ...
  function initGameLinks(){
    document.querySelectorAll('.game-card').forEach(card=>{
      ...
      button.addEventListener('click',()=>openGame(title));
    });
    document.querySelectorAll('[data-tm-game]').forEach(button=>{
      const key=button.dataset.tmGame?.trim();
      if(!GAME_ROUTES[key]||button.dataset.tmGameReady)return;
      button.dataset.tmGameReady='true';
      button.type='button';
      button.addEventListener('click',()=>openGame(key));
    });
  }
